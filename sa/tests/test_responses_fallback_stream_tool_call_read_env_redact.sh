...
CHAT_SSE = "\n".join([
    'data: {"choices":[{"delta":{"tool_calls":[{"id":"call_read_env","type":"function","function":{"name":"read","arguments":"{\\"filePath\\":\\"/tmp/demo/.env\\"}"},"index":0}]},"finish_reason":null}]}',
    "",
...
(cd "${sa_dir}" && setsid ./hubproxy > /tmp/hubproxy_sa_responses_fallback_stream_tool_call_read_env_redact.log 2>&1 < /dev/null) &
hub_pid=$!
...
if tool.get("name") != "exec_command":
    raise SystemExit(f"read was not normalized: {tool}")
args = json.loads(tool.get("arguments") or "{}")
cmd = args.get("cmd") or ""
if not cmd.startswith("sed -E 's/(OPENAI_API_KEY|AUTH|TOKEN|KEY|SECRET)=.*/"):
    raise SystemExit(f"read .env was not redacted: {tool}")
if "<redacted>" not in cmd or "/tmp/demo/.env" not in cmd:
    raise SystemExit(f"redaction command missing expected parts: {tool}")
if cmd.startswith("cat "):
    raise SystemExit(f"read .env used cat: {tool}")
PY

if rg -q '"name":"read"|"choices"|data: \\[DONE\\]' "${response_body}"; then
  echo "chat fallback read .env leaked raw fields" >&2
  cat "${response_body}" >&2
  exit 1
fi

echo "responses_fallback_stream_tool_call_read_env_redact_ok"
