...
http.server.ThreadingHTTPServer(("127.0.0.1", 28089), Handler).serve_forever()
PY
server_pid=$!

for _ in {1..50}; do
  if ss -ltnp | rg -q '127\.0\.0\.1:28089'; then
    break
  fi
  sleep 0.1
done
...
awk '
  ...
  /^CHAT_BASE_URL=/ { print "CHAT_BASE_URL=http://127.0.0.1:28089/v1"; wrote_chat=1; next }
  /^RESPONSES_BASE_URL=/ { print "RESPONSES_BASE_URL=http://127.0.0.1:28089/v1"; wrote_resp=1; next }
  /^SA_PORT=/ { print "SA_PORT=28080"; wrote_sa_port=1; next }
  { print }
  END {
    if (!wrote_chat) print "CHAT_BASE_URL=http://127.0.0.1:28089/v1"
    if (!wrote_resp) print "RESPONSES_BASE_URL=http://127.0.0.1:28089/v1"
    if (!wrote_sa_port) print "SA_PORT=28080"
  }
' "${backup_file}" >"${env_file}"

(cd "${sa_dir}" && setsid ./hubproxy > /tmp/hubproxy_sa_responses_fallback_stream_tool_call_split.log 2>&1 < /dev/null) &
hub_pid=$!
...
if tool.get("call_id") != "call_pwd":
    raise SystemExit(f"wrong call id: {tool}")
if tool.get("name") != "exec_command":
    raise SystemExit(f"wrong tool name: {tool}")
if tool.get("arguments") != '{"cmd":"pwd"}':
    raise SystemExit(f"split arguments were not merged: {tool}")
PY

if rg -q '"choices"|data: \\[DONE\\]|Tool unknown is unavailable' "${response_body}"; then
  echo "chat fallback leaked raw chat completions SSE or unknown tool notice" >&2
  cat "${response_body}" >&2
  exit 1
fi

echo "responses_fallback_stream_tool_call_split_ok"
