...
curl -sS --max-time 15 \
  ...
  --data '{"model":"models/mimo-v2.5-pro","stream":true,"tools":[{"type":"function","name":"update_plan","parameters":{}}],"input":[{"type":"message","role":"developer","content":[{"type":"input_text","text":"<goal_context>Continue working toward the active thread goal.</goal_context>"}]},{"type":"message","role":"user","content":[{"type":"input_text","text":"hello"}]}]}' \
...
if rg -q 'Progress-only message received in chat fallback|"name":"exec_command"|"choices"|data: \[DONE\]' "${response_body}"; then
  echo "plan/goal progress fallback injected continuation without exec_command tool or leaked raw SSE" >&2
  cat "${response_body}" >&2
  exit 1
fi

echo "responses_fallback_stream_progress_no_exec_tool_ok"
