...
http.server.ThreadingHTTPServer(("127.0.0.1", 28085), Handler).serve_forever()
PY
server_pid=$!

for _ in {1..50}; do
  if ss -ltnp | rg -q '127\.0\.0\.1:28085'; then
    break
  fi
  sleep 0.1
done
...
awk '
  ...
  /^CHAT_BASE_URL=/ { print "CHAT_BASE_URL=http://127.0.0.1:28085/v1"; wrote_chat=1; next }
  /^RESPONSES_BASE_URL=/ { print "RESPONSES_BASE_URL=http://127.0.0.1:28085/v1"; wrote_resp=1; next }
  /^SA_PORT=/ { print "SA_PORT=28080"; wrote_sa_port=1; next }
  { print }
  END {
    if (!wrote_chat) print "CHAT_BASE_URL=http://127.0.0.1:28085/v1"
    if (!wrote_resp) print "RESPONSES_BASE_URL=http://127.0.0.1:28085/v1"
