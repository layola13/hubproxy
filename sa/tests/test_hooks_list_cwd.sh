...
(
  cd "${sa_dir}"
  env PWD=/tmp/not-real-pwd setsid "${sa_dir}/hubproxy" > /tmp/hubproxy_sa_hooks_list_cwd.log 2>&1 < /dev/null &
  echo "$!" > /tmp/hubproxy_sa_hooks_list_cwd.pid
)
hub_pid="$(cat /tmp/hubproxy_sa_hooks_list_cwd.pid)"
rm -f /tmp/hubproxy_sa_hooks_list_cwd.pid
...
if ! rg -q "\"cwd\":\"${sa_dir}\"" <<<"${response}"; then
  echo "hooks/list did not use runtime Deno.cwd()" >&2
  echo "expected cwd: ${sa_dir}" >&2
  echo "${response}" >&2
  exit 1
fi
